...
def render(opt, generator_path, out_path):
    opt.generator_path = generator_path
...
def main():
    ...
    p.add_argument("--baseline_generator_path", required=True)
    ...
    p.add_argument("--cam_condition_mode", type=str, default="normal", choices=["normal", "remove", "zero_embedding"])
    opt = p.parse_args()
    opt.ngpus = 1

    os.makedirs(opt.out_dir, exist_ok=True)
    original = os.path.join(opt.out_dir, f"{opt.name}_original.mp4")
    trained = os.path.join(opt.out_dir, f"{opt.name}_{opt.cam_condition_mode}.mp4")
    combined = os.path.join(opt.out_dir, f"{opt.name}_original_vs_{opt.cam_condition_mode}.mp4")

    print("[preview] rendering original", flush=True)
    render(opt, opt.baseline_generator_path, original)
    print("[preview] rendering trained", flush=True)
    render(opt, opt.checkpoint, trained)
    hstack([original, trained], ["original", opt.cam_condition_mode], combined)
