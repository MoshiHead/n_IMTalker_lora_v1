...
class TrainOptions(BaseOptions):
    ...
    def initialize(self, parser):
        ...
        parser.add_argument("--cam_condition_mode", type=str, default="normal", choices=["normal", "remove", "zero_embedding"])
...
class DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        self.train_dataset = AudioMotionSmirkGazeDataset(opt=self.opt, start=0, end=-100, list_path=getattr(self.opt, "list_path", None))
        self.val_dataset = AudioMotionSmirkGazeDataset(opt=self.opt, start=-100, end=-1, list_path=getattr(self.opt, "list_path", None))
...
if __name__ == '__main__':
    ...
    trainer = pl.Trainer(
        ...
        max_steps=opt.iter,
        max_time={"minutes": opt.max_minutes} if int(getattr(opt, "max_minutes", 0)) > 0 else None,
