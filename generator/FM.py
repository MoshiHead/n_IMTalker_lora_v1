...
class FMGenerator(nn.Module):
    ...
    def forward(self, batch, t):
        ...
        cam_mode = getattr(self.opt, "cam_condition_mode", "normal")
        if cam_mode in ("remove", "zero_embedding"):
            # remove: FMT omits cam from c; zero_embedding: FMT receives explicit zero embedding.
            cam = torch.zeros_like(pose)
            prev_cam = torch.zeros_like(prev_pose)
        else:
