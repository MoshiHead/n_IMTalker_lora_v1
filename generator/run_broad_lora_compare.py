...
def load_lora(agent, lora_path):
    state = torch.load(lora_path, map_location="cpu")
    mode = state["lora_mode"]
    rank = int(state["lora_rank"])
    alpha = float(state["lora_alpha"])
    lora_state = state["lora_state_dict"]
    replaced = apply_lora_to_model(
        agent.fm,
        rank=rank,
        alpha=alpha,
        dropout=0.0,
        include_pose_lora=False,
        include_fmt_lora=False,
        lora_mode=mode,
    )
    missing, unexpected = agent.fm.load_state_dict(lora_state, strict=False)
    print(
        f"[LoRA] {os.path.basename(os.path.dirname(os.path.dirname(lora_path)))} "
        f"mode={mode} rank={rank} alpha={alpha} replaced={len(replaced)} "
...
def main():
    ...
    parser.add_argument("--exp1_lora", required=True)
    parser.add_argument("--exp2_lora", required=True)
    parser.add_argument("--exp3_lora", required=True)
    ...
    paths = [
        ...
        os.path.join(opt.out_dir, "exp1_no_audio_gaze_r8.mp4"),
        os.path.join(opt.out_dir, "exp2_no_audio_r8.mp4"),
        os.path.join(opt.out_dir, "exp3_no_audio_r64.mp4"),
    ]
    labels = ["original", "exp1_no_audio_gaze_r8", "exp2_no_audio_r8", "exp3_no_audio_r64"]

    for label, path, lora in zip(labels, paths, [None, opt.exp1_lora, opt.exp2_lora, opt.exp3_lora]):
        print(f"[RUN] {label}")
        render(opt, path, lora)
        torch.cuda.empty_cache()

    combined = os.path.join(opt.out_dir, "original_exp1_exp2_exp3_side_by_side.mp4")
    hstack(paths, labels, combined)
