...
def load_lora(agent, mode, lora_path, rank, alpha, dropout):
    include_fmt = mode == "pose_fmt"
    ...
    print(f"[LoRA] mode={mode} replaced={len(replaced)} missing={len(missing)} unexpected={len(unexpected)}")


def render_one(opt, out_path, lora_mode=None, lora_path=None):
...
def hstack_videos(paths, out_path):
    inputs = []
    labels = ["original", "pose_lora", "pose_fmt_lora"]
    for p in paths:
        inputs.extend(["-i", p])
    ...
    for idx, label in enumerate(labels):
        draw_filters.append(
            ...
            f"box=1:boxcolor=black@0.45:boxborderw=8[v{idx}]"
        )
    filter_complex = ";".join(draw_filters) + ";[v0][v1][v2]hstack=inputs=3[v]"
    cmd = [
        "ffmpeg",
        "-y",
        *inputs,
...
def main():
    ...
    p.add_argument("--pose_lora_path", required=True)
    p.add_argument("--pose_fmt_lora_path", required=True)
    ...
    p.add_argument("--use_all_wav2vec_layers", dest="only_last_features", action="store_false")
    ...
    original = os.path.join(args.out_dir, "original.mp4")
    pose = os.path.join(args.out_dir, "pose_lora.mp4")
    pose_fmt = os.path.join(args.out_dir, "pose_fmt_lora.mp4")
    combined = os.path.join(args.out_dir, "original_pose_posefmt_side_by_side.mp4")

    print("[RUN] original")
    render_one(args, original)
    torch.cuda.empty_cache()

    print("[RUN] pose_lora")
    render_one(args, pose, lora_mode="pose", lora_path=args.pose_lora_path)
    torch.cuda.empty_cache()

    print("[RUN] pose_fmt_lora")
    render_one(args, pose_fmt, lora_mode="pose_fmt", lora_path=args.pose_fmt_lora_path)
    torch.cuda.empty_cache()

    hstack_videos([original, pose, pose_fmt], combined)
