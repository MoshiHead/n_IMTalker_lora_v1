...
class AudioMotionSmirkGazeDataset(Dataset):
    def __init__(self, opt, start, end, list_path=None):
        super().__init__()
        self.opt = opt
        ...
        audio_dir = root_path / "audio_wav2vec"
        ...
        if list_path and Path(list_path).exists():
            ...
            with open(list_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if not stem:
                        continue
                    mp = motion_dir / f"{stem}.pt"
                    if mp.exists():
                        motion_files.append(mp)
        else:
        ...
        self.samples = []
        skip_dataset_filter = bool(getattr(self.opt, "skip_dataset_filter", False))
