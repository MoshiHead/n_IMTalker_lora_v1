...
def load_lora(agent, lora_path):
    ...
    print(
        f"[LoRA] mode={mode} rank={rank} alpha={alpha} "
...
def main():
    ...
    original = os.path.join(opt.out_dir, "original.mp4")
    lora = os.path.join(opt.out_dir, "no_audio_projection_rest_lora_r64_3000.mp4")
    combined = os.path.join(opt.out_dir, "original_vs_no_audio_projection_rest_lora_r64_3000.mp4")

    print("[RUN] original")
    render(opt, original)
    torch.cuda.empty_cache()

    print("[RUN] no_audio_projection_rest_lora")
    render(opt, lora, opt.lora_path)
    torch.cuda.empty_cache()

    hstack(
        [original, lora],
        ["original", "no_audio_projection_rest_lora_r64_3000"],
        combined,
    )
